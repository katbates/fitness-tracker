...
## Categorising type of user
Categorise users into activity level by number of steps: high activity =  more than 10,000 steps, medium activity = between 5,000 and 10,000 steps, low activity = less than 5,000 steps
```{r echo=TRUE, message=FALSE, warning=FALSE}
av_daily_steps <- daily_all_clean %>%
  group_by(id) %>%
  summarise(av_steps=mean(total_steps))

av_daily_steps$user_type <- as.factor(ifelse(av_daily_steps$av_steps>=10000, "High activity", ifelse(av_daily_steps$av_steps<10000 & av_daily_steps$av_steps>=5000, "Medium activity", "Low activity")))

daily_all_clean <- left_join(daily_all_clean, av_daily_steps, by="id")

```

```{r echo=TRUE, message=FALSE, warning=FALSE}
daily_all_clean <- mutate(daily_all_clean, sleep_hours = total_minutes_asleep/60)

```

## Plot distance, calories and sleep by usertype
```{r echo=TRUE, message=FALSE, warning=FALSE}
ggarrange(
  ggplot(daily_all_clean, aes(x= reorder(user_type, -calories), y=calories, fill=user_type)) +
  stat_summary_bin(fun="mean", geom ="bar") +
  scale_fill_manual(values=c("skyblue4", "skyblue3", "skyblue"))+
  theme_classic() +
  labs(title="Average calories by user type", x="User type", y="Average calories") +
  theme(legend.position = "none"),
  
  ggplot(daily_all_clean, aes(x= reorder(user_type, -tracker_distance_km), y=tracker_distance_km, fill=user_type)) +
  stat_summary_bin(fun="mean", geom ="bar") +
  scale_fill_manual(values=c("skyblue4", "skyblue3", "skyblue"))+
  theme_classic() +
  labs(title="Average distance travelled by user type", x="User type", y="Average distance")+
  theme(legend.position = "none"),
  
  ggplot(daily_all_clean, aes(x= reorder(user_type, -sleep_hours), y=sleep_hours, fill=user_type)) +
  stat_summary_bin(fun="mean", geom ="bar") +
  scale_fill_manual(values=c("skyblue4", "skyblue3", "skyblue"))+
  theme_classic() +
  labs(title="Average sleep by user type", x="User type", y="Average hours of sleep")+
  theme(legend.position = "none")
)

```
#### Observations:
* Users who have high activity (based on step count) burn more calories, travel further and sleep less
...
ggplot(data=av_steps_per_hour) +
  geom_col(mapping=aes(x=time, y=average_steps), fill="skyblue") +
  theme_minimal() +
...
```
#### Observations:
* People are least active from 8pm to 11pm - targetted time for marketing
...
* Automatic features are more frequently used than manual features. For example, automatic step count used by more people than logging activities. 
* Sleep tracking used by only 1/3 of participants
* Users are least active between 8pm and 11pm, making them more susceptible to marketing in this time period
* FitBit is being used for a range of exercise activities - not just running. 
